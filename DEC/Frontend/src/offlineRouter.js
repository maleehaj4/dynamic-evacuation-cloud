...
import { getOfflineRoute } from './offlineRouter'
...
export default function App() {
  ...
  async function handleMapClick(lat, lng) {
    ...
    try {
      if (!navigator.onLine) {
        const result = await getOfflineRoute(lat, lng)
        setRoute(result.coords)
        setInfo({ km: '?', dest: result.destination + ' (offline)' })
      } else {
      ...
      const result = await getOfflineRoute(lat, lng)
      setRoute(result.coords)
      setInfo({ km: '?', dest: result.destination + ' (offline fallback)' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        padding: '12px 20px',
        background: offline ? '#E74C3C' : '#1E8449',
        color: 'white', fontFamily: 'sans-serif', fontSize: '14px'
      }}>
        {offline ? '🔴 OFFLINE MODE — Using cached routes' : '🟢 LIVE MODE — Real-time routing active'}
        {loading && '  ·  Computing route...'}
        {info && !info.error && `  ·  Route: ${info.km} km to ${info.dest}`}
        {info?.error && `  ·  ${info.error}`}
      </div>

      <div style={{ flex: 1 }}>
        <MapContainer center={[17.385, 78.487]} zoom={12} style={{ height: '100%', width: '100%' }}>
          <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="© OpenStreetMap" />
          <MapClickHandler onMapClick={handleMapClick} />

          {HAZARD_ZONES.map((hz, i) => (
            <Circle key={i} center={hz.center} radius={hz.radius}
              pathOptions={{ color: hz.color, fillOpacity: 0.25 }}>
          ...
          {route && <Polyline positions={route}
            pathOptions={{ color: '#27AE60', weight: 5, dashArray: '10 5' }} />}
        </MapContainer>
      </div>
    </div>
  )
